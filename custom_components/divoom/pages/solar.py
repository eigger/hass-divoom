...
def solar(pixoo, hass, page_data, FONT_PICO_8, FONT_GICKO):
    ...
    try:
        ...
        rendered_discharge = float(Template(page_data['discharge'], hass).async_render())
        rendered_powerhousetotal = float(Template(page_data['powerhousetotal'], hass).async_render())
        rendered_vomNetz = float(Template(page_data['vomNetz'], hass).async_render())
        rendered_time = str(Template(page_data['time'], hass).async_render())
    except TemplateError as e:
        _LOGGER.error("Template render error: %s", e)
        return  # Stop execution if there is a template error

    image_folder = "/config/custom_components/divoom/img/"
